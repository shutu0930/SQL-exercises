...
FROM world w1
WHERE continent IN
(SELECT continent FROM world w2 WHERE w1.continent = w2.continent GROUP BY continent HAVING MAX(population)<=25000000)
...
FROM world w1
WHERE population > ALL(SELECT population*3 FROM world w2 WHERE w1.continent = w2.continent && population>0 && w1.name!=w2.name)
